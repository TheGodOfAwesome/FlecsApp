...
function create_new_event(
    $event_name, $event_creator, $event_desc, $event_capacity, $event_wallet, $db
) {
	...
	if (pg_num_rows($event_result) > 0) {
		...
		$new_empty_list = "[]";
		$start_amt = 0;
		$event_chain = "avalanche";

		$query = "INSERT INTO events
		(id, event_id, event_name, event_creator, event_desc, event_wallet, event_chain, event_ticket_price, event_ticket_crypto, event_capacity, event_rsvps, created_at, updated_at, last_activity_timestamp)
		VALUES
		(DEFAULT, '" . $event_id . "','" . $event_name . "','" . $event_creator . "','" . $event_desc . "','" . $event_wallet . "', '" . $event_chain . "','" 
...
function get_latest_events($page_size, $db) {
	...
	$query = "SELECT * FROM events order by created_at limit " . $page_size . ";";
	$result = pg_query($db, $query);
	$events = pg_fetch_all($result);
	return $events;
}
